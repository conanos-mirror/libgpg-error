...
class LibgpgerrorConan(ConanFile):
    name = "libgpg-error"
    version = "1.33"
    description = "Libgpg-error is a small library with error codes and descriptions shared by most GnuPG related software"
    url = "https://github.com/conanos/libgpg-error"
    ...
    def source(self):
        url_ = 'https://github.com/ShiftMediaProject/libgpg-error/archive/libgpg-error-{version}.tar.gz'
    ...
    def build(self):
        if self.settings.os == 'Windows':
            with tools.chdir(os.path.join(self._source_subfolder,"SMP")):
                ...
                msbuild.build("libgpg-error.sln",upgrade_project=True,platforms={'x86': 'Win32', 'x86_64': 'x64'},build_type=build_type)
    ...
    def package(self):
        if self.settings.os == 'Windows':
            ...
            shutil.copyfile(os.path.join(self.build_folder,self._source_subfolder,"src","gpg-error.pc.in"),
                            os.path.join(self.package_folder,"lib","pkgconfig", "gpg-error.pc"))
            lib = "gpg-errord" if self.options.shared else "gpg-error"
            replacements = {
                ...
                "@GPG_ERROR_CONFIG_LIBS@"         : "-L{libdir} -l%s"%(lib)
            }
            for s, r in replacements.items():
                tools.replace_in_file(os.path.join(self.package_folder,"lib","pkgconfig", "gpg-error.pc"),s,r)
